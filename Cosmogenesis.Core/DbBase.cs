...
namespace Cosmogenesis.Core
{
    public abstract class DbBase
    {
        ...
        protected internal virtual Container Container { get; } = default!;
        protected internal virtual DbSerializerBase Serializer { get; } = default!;
        ...
        public async virtual Task ValidateContainer(CancellationToken cancellationToken)
        {
            ...
            {
                var prop = await containers.ReadNextAsync().ConfigureAwait(false);
        ...
        protected internal virtual async Task<CreateResult<T>> CreateItemAsync<T>(T item, string type, PartitionKey partitionKey, string partitionKeyString) where T : DbDoc
        {
            if (item is null)
            {
        ...
        protected internal virtual async Task<CreateOrReplaceResult<T>> CreateOrReplaceItemAsync<T>(T item, string type, PartitionKey partitionKey, string partitionKeyString) where T : DbDoc
        {
            if (item is null)
            {
                throw new ArgumentNullException(nameof(item));
        ...
        protected internal virtual async Task<ReadOrCreateResult<T>> ReadOrCreateItemAsync<T>(T item, string type, PartitionKey partitionKey, string partitionKeyString, bool tryCreateFirst) where T : DbDoc
        {
            if (item is null)
            {
                throw new ArgumentNullException(nameof(item));
        ...
        protected internal virtual async Task<ReplaceResult<T>> ReplaceItemAsync<T>(T item, string type, PartitionKey partitionKey, string partitionKeyString) where T : DbDoc
        {
            if (item is null)
            {
                throw new ArgumentNullException(nameof(item));
        ...
        protected internal virtual async Task<DbConflictType?> DeleteItemAsync<T>(T item, PartitionKey partitionKey, string partitionKeyString) where T : DbDoc
        {
            if (item is null)
            {
                throw new ArgumentNullException(nameof(item));
